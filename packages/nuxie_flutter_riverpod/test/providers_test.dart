...
void main() {
  ...
  test('nuxieFeatureProvider yields initial and update values', () async {
    ...
    const query = NuxieFeatureQuery('pro_feature');
    final emitted = <FeatureAccess?>[];

    final sub = container.listen<AsyncValue<FeatureAccess?>>(
      nuxieFeatureProvider(query),
      (_, next) {
        emitted.add(next.valueOrNull);
      },
